...
public class ExcelRowCopy extends RangeCopier {
    ...
    /*static String[] statementTypes = {
            "Income Statement_Annual_As Originally Reported (1).xls",
            "Balance Sheet_Annual_As Originally Reported (1).xls",
            "Cash Flow_Annual_As Originally Reported (1).xls"};*/
    static String [] statementTypes = {"Income", "Balance", "Cash"};
    //static StatementType statementType = new StatementType();
    ...
    public static void copyData() throws IOException {
        ...
        Workbook destinationWorkbook = getDestinationWorkbook();
            for(String statementType: statementTypes) {
                copyRows(getSourceWorkbook(StatementType.urls.get(statementType)), destinationWorkbook,
                        StatementType.cellRanges.get(statementType), /*mapTypes.get("source" + statementType)*/
                        StatementType.cellRanges.get(statementType)); //mapTypes.get("dest" + statementType));
            }
